...
public class AdapterPdfAdminl42 extends RecyclerView.Adapter<AdapterPdfAdminl42.HolderPdfAdmin> implements Filterable {//

    private final Context context;//context
    public ArrayList<ModelPdfl42> pdfArrayList,filterlist;//array holder
    private RowPdfAdminBinding binding;//vb
    private FilterPdfAdminl42 filter;
    private static final String TAG ="PDF_ADAPTER_TAG";
    private final ProgressDialog progressDialog;

    //con
    public AdapterPdfAdminl42(Context context1, ArrayList<ModelPdfl42> pdfArrayList) {
    ...
    private void moreOptionsDalog(ModelPdfl42 model, HolderPdfAdmin holder) {
        ...
        builder.setTitle("Choose Options")
                .setItems(options, new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        if (which==0){
                            //edit
                            Intent intent = new Intent(context, PdfEditActivityl42.class);
                            intent.putExtra("expId",expId);
                            context.startActivity(intent);
                        }
                        else if (which==1){
                            //delete
                            MyApplicationl42.deleteExp(
                                    context,
                                    ""+expId,
                                    ""+expUrl,
                                    ""+expTitle
...
//code is removed
    ...
    public Filter getFilter() {
        if (filter == null){
            filter = new FilterPdfAdminl42(filterlist,this);
        }
        return filter;
    }
