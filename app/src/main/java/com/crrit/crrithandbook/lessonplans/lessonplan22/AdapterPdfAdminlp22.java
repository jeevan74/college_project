...
public class AdapterPdfAdminlp22 extends RecyclerView.Adapter<AdapterPdfAdminlp22.HolderPdfAdmin> implements Filterable {//

    private final Context context;//context
    public ArrayList<ModelPdflp22> pdfArrayList,filterlist;//array holder
    private RowPdfAdminBinding binding;//vb
    private FilterPdfAdminlp22 filter;
    private static final String TAG ="PDF_ADAPTER_TAG";
    private final ProgressDialog progressDialog;

    //con
    public AdapterPdfAdminlp22(Context context1, ArrayList<ModelPdflp22> pdfArrayList) {
    ...
    private void moreOptionsDalog(ModelPdflp22 model, HolderPdfAdmin holder) {
        ...
        builder.setTitle("Choose Options")
                .setItems(options, new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        if (which==0){
                            //edit
                            Intent intent = new Intent(context, PdfEditActivitylp22.class);
                            intent.putExtra("expId",expId);
                            context.startActivity(intent);
                        }
                        else if (which==1){
                            //delete
                            MyApplicationlp22.deleteExp(
                                    context,
                                    ""+expId,
                                    ""+expUrl,
                                    ""+expTitle
...
//code is removed
    ...
    public Filter getFilter() {
        if (filter == null){
            filter = new FilterPdfAdminlp22(filterlist,this);
        }
        return filter;
    }
