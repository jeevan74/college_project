...
public class AdapterPdfAdminm42 extends RecyclerView.Adapter<AdapterPdfAdminm42.HolderPdfAdmin> implements Filterable {//

    private Context context;//context
    public ArrayList<ModelPdfm42> pdfArrayList,filterlist;//array holder
    private RowPdfAdminBinding binding;//vb
    private FilterPdfAdminm42 filter;
    private static final String TAG ="PDF_ADAPTER_TAG";
    private ProgressDialog progressDialog;

    //con
    public AdapterPdfAdminm42(Context context1, ArrayList<ModelPdfm42> pdfArrayList) {
    ...
    private void moreOptionsDalog(ModelPdfm42 model, HolderPdfAdmin holder) {
        ...
        builder.setTitle("Choose Options")
                .setItems(options, new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        if (which==0){
                            //edit
                            Intent intent = new Intent(context, PdfEditActivitym42.class);
                            intent.putExtra("expId",expId);
                            context.startActivity(intent);
                        }
                        else if (which==1){
                            //delete
                            MyApplicationm42.deleteExp(
                                    context,
                                    ""+expId,
                                    ""+expUrl,
                                    ""+expTitle
...
//code is removed
    ...
    public Filter getFilter() {
        if (filter == null){
            filter = new FilterPdfAdminm42(filterlist,this);
        }
        return filter;
    }
